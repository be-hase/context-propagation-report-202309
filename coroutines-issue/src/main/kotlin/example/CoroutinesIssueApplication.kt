...
import example.CoroutinesIssueApplication.MyThreadLocalAccessor
...
fun main(args: Array<String>) {
    ...
    runApplication<CoroutinesIssueApplication>(*args)
}

@SpringBootApplication
class CoroutinesIssueApplication {
        ...
        fun test(): String {
            ...
            runBlocking {
                webClient.get().uri("https://example.com").retrieve().toBodilessEntity()
                    .doOnSuccess {
                        log.info(
                            "doOnSuccess. MDC={}, myThreadLocal={}",
                            MDC.getCopyOfContextMap(), myThreadLocal.get()
                        )
                    }
                    // We need to write contextCapture each time to do context propagation
                    // .contextCapture()
                    .awaitSingle()
            }
            return "OK"
